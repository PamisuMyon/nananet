...
public class DodoBot : IBot
{
    ...
    public DodoBot(InitOptions? options = default)
    {
    ...
    public async Task Launch()
    {
        ...
        _mentionRegex = new Regex($"(<@!{appSettings.BotId}>|[@＠]{_me.NickName})", 
    ...
    {
        if (input.MessageType != (int)DodoMessageType.Text 
            || input.MessageType != (int)DodoMessageType.Picture)
            return;
    ...
    {
        return await SendTextMessage(to.IsPersonal ? to.AuthorId : to.ChannelId, content, to.IsPersonal, to.MessageId);
    }
    ...
    {
        return await SendPictureFileMessage(to.IsPersonal ? to.AuthorId : to.ChannelId, filePath, to.IsPersonal, to.MessageId);
    }
    ...
    {
        return await SendPictureUrlMessage(to.IsPersonal ? to.AuthorId : to.ChannelId, url, to.IsPersonal, to.MessageId);
    }
